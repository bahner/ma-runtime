...
default-config-root-populated = 起動時に既定の /config/root を設定しました
default-config-root-no-root-entity = 起動時に既定の /config/root を設定できません: #root エンティティが読み込まれていません
default-config-root-no-root-cid = 起動時に既定の /config/root を設定できません: マニフェストの root CID が利用できません
default-config-root-inspect-failed = 既定の /config/root を設定する前にマニフェストを検査できませんでした
default-config-root-populate-failed = 起動時に既定の /config/root を設定できませんでした
...
boot-default-root-config-skip = デフォルト設定ルートは既に設定されています
...
boot-runtime-kinds-overlay-failed = kinds CIDオーバーレイの適用に失敗しました
boot-runtime-manifest-load-failed = ACLキャッシュ入力用マニフェストの読み込みに失敗しました
boot-runtime-root-cid-resolve-failed = IPNSからランタイムルートCIDの解決に失敗しました
...
bootstrap-kind-registry-fetch-failed = レジストリ用kindノードの取得に失敗しました
...
inbox-behaviour-ignored = 受信ボックス: fire-and-forgetディスパッチでma_set_behaviourリクエストは無視されます
inbox-dispatching = 受信ボックス: エンティティへディスパッチ中
inbox-empty-payload-dropped = 受信ボックス: 空のペイロードを破棄しました
inbox-message-rejected = 受信ボックスのメッセージを拒否しました
inbox-unfragmented-dropped = 受信ボックス: 非フラグメントメッセージを破棄しました
inbox-unknown-entity = 受信ボックス: 不明なエンティティフラグメント。破棄しました
...
outbox-connect-backoff = アウトボックスの接続に失敗しました。フィボナッチバックオフを適用します
outbox-connect-failed-fallback = キャッシュされたアウトボックスの接続に失敗しました。リゾルバにフォールバックします
outbox-connect-timed-out-fallback = キャッシュされたアウトボックスの接続がタイムアウトしました。リゾルバにフォールバックします
outbox-connect-timeout-backoff = アウトボックスの接続がタイムアウトしました。フィボナッチバックオフを適用します
plugin-dispatch-rejected = プラグインディスパッチを拒否しました
...
plugin-envelope-local-reply-dropped = プラグインエンベロープ: ローカルRPC返信を破棄しました（ローカル返信待機者なし）
