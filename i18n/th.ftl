...
default-config-root-populated = เติมค่าเริ่มต้น /config/root แล้วเมื่อเริ่มต้น
default-config-root-no-root-entity = ไม่สามารถเติมค่าเริ่มต้น /config/root เมื่อเริ่มต้นได้: ยังไม่ได้โหลดเอนทิตี #root
default-config-root-no-root-cid = ไม่สามารถเติมค่าเริ่มต้น /config/root เมื่อเริ่มต้นได้: ไม่มี CID รากของ manifest
default-config-root-inspect-failed = ตรวจสอบ manifest ก่อนเติมค่าเริ่มต้น /config/root ไม่สำเร็จ
default-config-root-populate-failed = เติมค่าเริ่มต้น /config/root เมื่อเริ่มต้นไม่สำเร็จ
...
owners-value-not-list = ค่า owners ต้องเป็นรายการของ DID ไม่ใช่ค่าเดียว
...
boot-default-root-config-skip = การตั้งค่า root เริ่มต้นถูกตั้งค่าไว้แล้ว
...
boot-runtime-kinds-overlay-failed = ไม่สามารถใช้ Kinds CID overlay ได้
boot-runtime-manifest-load-failed = ไม่สามารถโหลด manifest เพื่อเติม ACL cache ได้
boot-runtime-root-cid-resolve-failed = ไม่สามารถ resolve runtime root CID จาก IPNS ได้
...
bootstrap-kind-registry-fetch-failed = ไม่สามารถดึง Kind node สำหรับ registry ได้
...
inbox-behaviour-ignored = inbox: ละเว้นคำขอ ma_set_behaviour เมื่อส่งแบบ fire-and-forget
inbox-dispatching = inbox: กำลังส่งไปยัง entity
inbox-empty-payload-dropped = inbox: ทิ้ง payload ว่าง
inbox-message-rejected = ปฏิเสธข้อความ inbox
inbox-unfragmented-dropped = inbox: ทิ้งข้อความที่ไม่มีการแบ่งส่วน
inbox-unknown-entity = inbox: fragment ของ entity ที่ไม่รู้จัก; ทิ้งแล้ว
...
outbox-connect-backoff = การเชื่อมต่อ outbox ล้มเหลว; กำลังใช้ Fibonacci backoff
outbox-connect-failed-fallback = การเชื่อมต่อ outbox จาก cache ล้มเหลว; กลับไปใช้ resolver
outbox-connect-timed-out-fallback = การเชื่อมต่อ outbox จาก cache หมดเวลา; กลับไปใช้ resolver
outbox-connect-timeout-backoff = การเชื่อมต่อ outbox หมดเวลา; กำลังใช้ Fibonacci backoff
plugin-dispatch-rejected = ปฏิเสธ plugin dispatch
...
plugin-envelope-local-reply-dropped = plugin envelope: ทิ้ง local RPC reply (ไม่มี local reply waiter)
